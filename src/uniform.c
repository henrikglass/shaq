...
i32 uniform_parse_from_ini_kv_pair(Uniform *u, HglIniKVPair *kv)
{
    ...
    if (!sv_lchop_if_starts_with(&k, "uniform")) {
    ...
    k = sv_ltrim(k);
    if      (sv_lchop_if_starts_with(&k, "bool"))      { u->type = TYPE_BOOL;    }
    else if (sv_lchop_if_starts_with(&k, "int"))       { u->type = TYPE_INT;     }
    else if (sv_lchop_if_starts_with(&k, "uint"))      { u->type = TYPE_UINT;    }
    else if (sv_lchop_if_starts_with(&k, "float"))     { u->type = TYPE_FLOAT;   }
    else if (sv_lchop_if_starts_with(&k, "vec2"))      { u->type = TYPE_VEC2;    }
    else if (sv_lchop_if_starts_with(&k, "vec3"))      { u->type = TYPE_VEC3;    }
    else if (sv_lchop_if_starts_with(&k, "vec4"))      { u->type = TYPE_VEC4;    }
    else if (sv_lchop_if_starts_with(&k, "ivec2"))     { u->type = TYPE_IVEC2;   }
    else if (sv_lchop_if_starts_with(&k, "ivec3"))     { u->type = TYPE_IVEC3;   }
    else if (sv_lchop_if_starts_with(&k, "ivec4"))     { u->type = TYPE_IVEC4;   }
    else if (sv_lchop_if_starts_with(&k, "mat2"))      { u->type = TYPE_MAT2;    }
    else if (sv_lchop_if_starts_with(&k, "mat3"))      { u->type = TYPE_MAT3;    }
    else if (sv_lchop_if_starts_with(&k, "mat4"))      { u->type = TYPE_MAT4;    }
    else if (sv_lchop_if_starts_with(&k, "sampler2D")) { u->type = TYPE_TEXTURE; }
    else {
